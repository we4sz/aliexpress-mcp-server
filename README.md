...
- **Whole-cart selection through this API tops out around 20 ticked lines, and long selection calls block the server.** Ticking a line reliably knocks another off server-side; five cart sizes (22, 25, 26, 28, 35 lines) all settled at exactly 20 ticked, with repeat calls making no progress. `set_cart_selection` converges what it can, detects when the total stops rising, and then says so instead of advising a retry that cannot work — use the site or app's own **Select all**, which is not subject to this. Separately, writes are paced ~5s apart to stay under the anti-bot threshold, so 35 lines would need ~175s; the tool stops at a wall-clock budget (`ALIEXPRESS_SELECTION_BUDGET`, default 45s) and returns a partial report rather than overrunning a caller's timeout. That matters because this server handles one call at a time — an over-running selection call stalls every other tool call behind it.
