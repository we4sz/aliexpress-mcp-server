...
- **The SAVED cart selection tops out around 20 lines, and long selection calls block the server.** Ticking a line reliably knocks another off server-side; five cart sizes (22, 25, 26, 28, 35 lines) all settled at exactly 20 ticked, with repeat calls making no progress. This is not a fault in this server, and the site's own **Select all** does not persist past it either: it renders every line ticked, but reload the cart and only ~20 survive. Nothing announces the limit; the render's own `reachLimitNotice` stays `false`. Note this does NOT appear to limit an *order* — checking out a larger cart from the site in one sitting reportedly works, so the checkout flow does not depend on the saved selection the way this server must. The practical consequence is only that this tool cannot hold more than ~20 lines ticked. `set_cart_selection` converges what it can, detects when the total stops rising, and says so rather than advising a retry that cannot work. Separately, writes are paced ~5s apart to stay under the anti-bot threshold, so 35 lines would need ~175s; the tool stops at a wall-clock budget (`ALIEXPRESS_SELECTION_BUDGET`, default 45s) and returns a partial report rather than overrunning a caller's timeout. That matters because this server handles one call at a time — an over-running selection call stalls every other tool call behind it.
